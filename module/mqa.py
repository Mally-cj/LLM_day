...
class MultiQueryAttention(nn.Module):
    def __init__(self, hidden_dim, num_heads=8):
        ...
        self.linear_k = nn.Linear(hidden_dim, self.head_dim)
        self.linear_v = nn.Linear(hidden_dim, self.head_dim)
    ...
    def forward(self, x):
        ...
        k = self.linear_k(x).reshape(bsz, seq_len, 1, d).transpose(1, 2)  # (bsz, 1, seq_len, d)
        v = self.linear_v(x).reshape(bsz, seq_len, 1, d).transpose(1, 2)  # (bsz, 1, seq_len, d)

        # b nh s d * b 1 d s = b nh s s
        ...
        # bsz, nh, s, s * bsz, 1, s, d = bsz, nh, s, d
